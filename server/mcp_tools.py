...
async def semantic_search_with_filters(
    ...
    try:
        ...
        if created_after:
            conditions.append({"created": {"$gte": created_after}})
        if created_before:
            conditions.append({"created": {"$lte": created_before}})

        if added_after:
            conditions.append({"added": {"$gte": added_after}})
        if added_before:
            conditions.append({"added": {"$lte": added_before}})
        ...
        if not docs:
            return "No relevant semantic chunks found."
        ...
        for doc, meta, dist in zip(docs, metas, distances):
            ...
            created_str = meta.get("created", "Unknown")
            ...
            output.append("\n---")
            output.append(f"### 📄 [{title} (ID: {doc_id})]({source_url})")
            output.append(f"**Similarity Score:** {1-dist:.4f} | **Correspondent:** {corr_name} | **Created:** {created_str}")
...
@mcp.tool()
async def get_document_details(document_id: int) -> str:
    """
